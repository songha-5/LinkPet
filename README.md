...
기능적 구조를 참고하는 것에 그치지 않고, 기존 서비스의 UI/UX를 사용자 친화적으로 개편 하였으며, 스토어중심의 웹사이트 보다 ***반려동물을 보다 케어하는 웹사이트로*** 작업을 하였습니다. 상태 관리와 데이터 시각화, 그리고 사용자 권한 분리를 직접 설계하고 구현하는 데 집중했습니다.
...
- **소개**:
  ...
  LinkPet은 보호자가 입력한 데이터를 바탕으로 직관적인 건강 점수와 시각화된 차트, 반려동물의 일정관리를 제공하여 반려동물의 상태를 쉽게 파악할 수 있도록 돕습니다. 
