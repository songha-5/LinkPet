  ...
  return (
    <>
      ...
      <div className="flex gap-2 pbs-4">
        <BaseButton onClick={handleFileUpload} content={isUploading ? (<><LoaderCircle className="animate-spin w-5 h-5 mr-3" /> 로딩중</>) : buttonText} color={color} className="text-[18px] h-14!" disabled={file ? false : true} />
      </div>
    </>
  )
}