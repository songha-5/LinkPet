...
export default function BaseRadio({ name, value, content, className, onClick, checked }: BaseRadioProps) {
  const id = useId()

  return (
    <div className={`flex-1 ${className}`} onClick={onClick}>
      <input checked={checked} className="peer sr-only" id={id} type="radio" name={name} value={value} />
