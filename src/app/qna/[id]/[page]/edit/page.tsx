...
export default async function EditPage({ params }: EditPageProps) {
  ...
  const { data: petData } = await supabase.from('pet').select('name, age, weight').eq('user_id', user.id)
  ...
  return (
    <>
      ...
      <div className="border-5 p-7 bg-bg border-neonPink shadow-[6px_6px_0_var(--color-neonPink)]">
        ...
        {/* 수정 폼 */}
        <EditForm id={paramPage} title={postDate?.title} body={postDate?.body} petName={petData?.[0].name} petAge={petData?.[0].age} petWeight={petData?.[0].weight} />
      </div>
    </>
  )
}